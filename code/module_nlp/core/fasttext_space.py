...
def get_wheels(config):
    ...
    select_words = {"Colors": color_words, "Emotions": emotion_words}
    ...
    for index, row in coords.iterrows():
        ...
        # difference vectors
        coords.loc[index, "Valence"] = current_word @ dimensions["Valence"][0]
        coords.loc[index, "Arousal"] = current_word @ dimensions["Arousal"][0]

        df_colors = coords.iloc[0 : len(color_words), :]
        df_colors[["z_valence", "z_arousal"]] = (
            df_colors[["Valence", "Arousal"]].astype(np.float64).apply(zscore, axis=0)
        )
        df_colors["Condition"] = list(
            itertools.repeat("Color", len(select_words["Colors"]))
        )
        df_emotions = coords.tail(len(select_words["Emotions"]))
        df_emotions[["z_valence", "z_arousal"]] = (
            df_emotions[["Valence", "Arousal"]].astype(np.float64).apply(zscore, axis=0)
        )
        df_emotions["Condition"] = list(
            itertools.repeat("Emotion", len(select_words["Emotions"]))
        )
    ...
    ce_distance = spatial.distance.cdist(
        df_emotions[["z_valence", "z_arousal"]].values.astype("float"),
        df_colors[["z_valence", "z_arousal"]].values.astype("float"),
        metric="euclidean",
    )

    en_colors = ["green", "orange", "yellow", "blue", "purple", "red"]
    distances = pd.DataFrame(
        1 / ce_distance, index=select_words["Emotions"], columns=en_colors
    )
    rgb_colors = {
    ...
    for i in distances.index:
        sorted_row = (
            distances[list(rgb_colors.keys())]
            .sort_values(i, axis=1, ascending=False)
            .loc[[i]]
        )
...
def control_rdm(config: dict) -> None:
    ...
    for words in select_words:
        ...
        rdm_fasttext_df = pd.DataFrame(
            data=rdm_fasttext, index=select_words[words], columns=select_words[words]
        )
        ...
        rdm_fasttext2d_df = pd.DataFrame(
            data=rdm_fasttext2d, index=select_words[words], columns=select_words[words]
        )

        # Compute RDM based on euclidean distance in the control space
        ## Select words
        control_coords.set_index("index", inplace=True)
        coords_geo = control_coords.loc[
            filter(lambda x: x in select_words[words], control_coords.index.values)
        ]
        ...
        for i in select_words[words]:
            coords_geo["ordering"].loc[coords_geo["index"] == i] = count
            count += 1
        ...
        rdm_control = pairwise.euclidean_distances(
            coords_geo[["Valence", "Arousal"]].values
        )
        rdm_control_df = pd.DataFrame(
            data=rdm_control, index=select_words[words], columns=select_words[words]
        )
...
def osgood_rdm(config) -> None:
    ...
    def stat_test(
        ...
        corr_control = spearmanr(rdm_human, rdm_control)
        ...
        for p in range(s):
            ...
            permute_osgoodspace[p] = spearmanr(
                human_sample, np.random.permutation(rdm_osgoodspace)
            )[0]
            permute_fasttext[p] = spearmanr(
                human_sample, np.random.permutation(rdm_fasttext)
            )[0]
            permute_fasttext2d[p] = spearmanr(
                human_sample, np.random.permutation(rdm_fasttext2d)
            )[0]
            permute_control[p] = spearmanr(
                human_sample, np.random.permutation(rdm_control)
            )[0]

        # Get p-value
        ## Fasttext
        diff_null = permute_osgoodspace - permute_fasttext
        diff_corr = corr_osgoodspace[0] - corr_fasttext[0]
        diff_corr_z = statistics.NormalDist(
            mu=np.mean(diff_null), sigma=np.std(diff_null)
        ).zscore(diff_corr)

        p_val = stats.norm.sf(abs(diff_corr_z)) * 2

        ## Fasttext2d
        diff_null2d = permute_osgoodspace - permute_fasttext2d
        diff_corr2d = corr_osgoodspace[0] - corr_fasttext2d[0]
        diff_corr_z2d = statistics.NormalDist(
            mu=np.mean(diff_null2d), sigma=np.std(diff_null2d)
        ).zscore(diff_corr2d)

        p_val2d = stats.norm.sf(abs(diff_corr_z2d)) * 2

        ## Control space
        diff_nullcont = permute_osgoodspace - permute_control
        diff_corrcont = corr_osgoodspace[0] - corr_control[0]
        diff_corr_zcont = statistics.NormalDist(
            mu=np.mean(diff_nullcont), sigma=np.std(diff_nullcont)
        ).zscore(diff_corrcont)

        p_valcont = stats.norm.sf(abs(diff_corr_zcont)) * 2

        corr_df = pd.DataFrame(
            data=[
                [corr_osgoodspace[0], corr_osgoodspace[1]],
        ...
        osgood_flatten = pd.DataFrame(
            {
                "Human ratings": rdm_human.flatten(),
                "Osgoodspace": rdm_osgoodspace.flatten(),
            }
        )
        fasttext_flatten = pd.DataFrame(
            {"Human ratings": rdm_human.flatten(), "Fasttext": rdm_fasttext.flatten()}
        )
        fasttext2d_flatten = pd.DataFrame(
            {
                "Human ratings": rdm_human.flatten(),
                "Fasttext2D": rdm_fasttext2d.flatten(),
            }
        )
        control_flatten = pd.DataFrame(
            {
                "Human ratings": rdm_human.flatten(),
    ...
    for words in select_words:
        ...
        coords = osgoodaxes_df.loc[
            filter(lambda x: x in select_words[words], osgoodaxes_df.index.values)
        ]
        ...
        rdm_osgoodspace = pairwise.euclidean_distances(
            coords[["Valence", "Arousal"]].values
        )
        # Test for normality
        osgood_norm = ce.test_normality(
            ce.del_upper(rdm_osgoodspace), title="Osgood space"
        )
        ...
        if words == "Colors":
            ...
            # Test for normality
            human_norm = ce.test_normality(
                ce.del_upper(rdm_human), title="Human ratings"
            )

            # Load rdm based on cosine distance in the original 300D space
            rdm_fasttext = pd.read_csv(
                ".." + path2control_rdm + "Colors_fasttext300.csv"
            )
            rdm_fasttext.set_index(rdm_fasttext.columns[0], inplace=True)
            # Test for normality
            fasttext_norm = ce.test_normality(
                ce.del_upper(rdm_fasttext.values), title="Fasttext 300D"
            )

            # Load fasttext2d RDM
            rdm_fasttext2d = pd.read_csv(
                ".." + path2control_rdm + "Colors_fasttext2d.csv"
            )
            rdm_fasttext2d.set_index(rdm_fasttext2d.columns[0], inplace=True)
            # Test for normality
            fasttext2d_norm = ce.test_normality(
                ce.del_upper(rdm_fasttext2d.values), title="Fasttext 2D"
            )

            # Load controlspace RDM
            rdm_control = pd.read_csv(
                ".." + path2control_rdm + "Colors_controlspace.csv"
            )
            rdm_control.set_index(rdm_control.columns[0], inplace=True)
            # Test for normality
            control_norm = ce.test_normality(
                ce.del_upper(rdm_control.values), title="Control space"
            )

            corr_method = "spearman"
            # Stats on color words
            diff_col, corr_col = stat_test(
                coords,
                rdm_human,
                rdm_osgoodspace,
                rdm_fasttext.values,
            ...
            # Load rdm based on cosine distance in the original 300D space
            rdm_fasttext = pd.read_csv(
                ".." + path2control_rdm + "Emotions_fasttext300.csv"
            )
            rdm_fasttext.set_index(rdm_fasttext.columns[0], inplace=True)
            # Test for normality
            fasttext_norm = ce.test_normality(
            ...
            # Load fasttext2d RDM
            rdm_fasttext2d = pd.read_csv(
                ".." + path2control_rdm + "Emotions_fasttext2d.csv"
            )
            rdm_fasttext2d.set_index(rdm_fasttext2d.columns[0], inplace=True)
            # Test for normality
            fasttext2d_norm = ce.test_normality(
            ...
            # Load controlspace RDM
            rdm_control = pd.read_csv(
                ".." + path2control_rdm + "Emotions_controlspace.csv"
            )
            rdm_control.set_index(rdm_control.columns[0], inplace=True)
            # Test for normality
            control_norm = ce.test_normality(
...
def coloremotion_associate(config: dict) -> pd.DataFrame:
    ...
    overall_df = pd.DataFrame(
        data=associations.values, index=associations.index, columns=associations.columns
    )
    ...
    euclidean_distances = pd.DataFrame(
        index=select_words["Emotions"], columns=select_words["Colors"]
    )

    for e in euclidean_distances.index.values:
        z_emotion = (
            centroids[["z_valence", "z_arousal"]].loc[centroids["index"] == e].values[0]
        )
        for c in euclidean_distances.columns.values:
            z_color = (
                centroids[["z_valence", "z_arousal"]]
                .loc[centroids["index"] == c]
                .values[0]
            )
    ...
    for e in cos_distances.index.values:
        for c in cos_distances.columns.values:
            cos_distances[c][e] = spatial.distance.cosine(
                em_embeddings[e], col_embeddings[c]
            )
    ...
    embeddings_2d = model_2d.model.loc[
        filter(lambda x: x in words_flat, model_2d.model.index.values)
    ]
    ...
    df_colors = pd.DataFrame(index=select_words["Colors"], columns=["0", "1"])
    ...
    df_emotions = pd.DataFrame(index=select_words["Emotions"], columns=["0", "1"])
    ...
    for e in rdm_fasttext2d.index.values:
        ...
        for c in rdm_fasttext2d.columns.values:
            ...
            rdm_fasttext2d[c][e] = spatial.distance.euclidean(
    ...
    # Compute euclidean distance
    control_distances = pd.DataFrame(
        index=select_words["Emotions"], columns=select_words["Colors"]
    )

    for e in control_distances.index.values:
        z_emotion = (
            coords_control[["z_valence", "z_arousal"]]
            .loc[coords_control["index"] == e]
            .values[0]
        )
        for c in control_distances.columns.values:
            z_color = (
                coords_control[["z_valence", "z_arousal"]]
                .loc[coords_control["index"] == c]
                .values[0]
            )
    ...
    statistic = lambda x, y: spearmanr(x, y).correlation
    corr_lowdim = stats.spearmanr(
        overall_df.values.flatten(), euclidean_distances.values.flatten()
    )
    corr_300d = stats.spearmanr(
        overall_df.values.flatten(), cos_distances.values.flatten()
    )
    corr_2d = stats.spearmanr(
        overall_df.values.flatten(), rdm_fasttext2d.values.flatten()
    )
    corr_control = stats.spearmanr(
        overall_df.values.flatten(), control_distances.values.flatten()
    )
    ...
    for p in range(s):
        ...
        permute_lowdim[p] = spearmanr(
            human_sample, np.random.permutation(euclidean_distances.values.flatten())
        )[0]
        permute_300d[p] = spearmanr(
            human_sample, np.random.permutation(cos_distances.values.flatten())
        )[0]
        permute_2d[p] = spearmanr(
            human_sample, np.random.permutation(rdm_fasttext2d.values.flatten())
        )[0]
        permute_control[p] = spearmanr(
            human_sample, np.random.permutation(control_distances.values.flatten())
        )[0]
    ## Copmute pvalue in 300D
    diff_null = permute_lowdim - permute_300d
    diff_corr = corr_lowdim[0] - corr_300d[0]
    diff_z = statistics.NormalDist(
        mu=np.mean(diff_null), sigma=np.std(diff_null)
    ).zscore(diff_corr)
    ...
    diff_z2d = statistics.NormalDist(
        mu=np.mean(diff_null2d), sigma=np.std(diff_null2d)
    ).zscore(diff_corr2d)
    ...
    diff_zcont = statistics.NormalDist(
        mu=np.mean(diff_nullcont), sigma=np.std(diff_nullcont)
    ).zscore(diff_corrcont)
    ...
    diff_df = pd.DataFrame(
        data=[[p_val, diff_corr], [p_val2d, diff_corr2d], [p_valcont, diff_corrcont]],
    ...
    osgood_flatten = pd.DataFrame(
        {
            "Human ratings": overall_df.values.flatten(),
    ...
    sns.lmplot(
        x="Human ratings",
        y="Osgoodspace",
        data=osgood_flatten.astype(float),
        height=7,
        aspect=1.2,
        robust=True,
        palette="tab10",
    ...
    sns.lmplot(
        x="Human ratings",
        y="Fasttext",
        data=fasttext_flatten.astype(float),
        height=7,
        aspect=1.2,
        robust=True,
        palette="tab10",
    ...
    sns.lmplot(
        x="Human ratings",
        y="Fasttext2D",
        data=fasttext2d_flatten.astype(float),
        height=7,
        aspect=1.2,
        robust=True,
        palette="tab10",
    ...
    sns.lmplot(
        x="Human ratings",
        y="Controlspace",
        data=control_flatten.astype(float),
        height=7,
        aspect=1.2,
        robust=True,
        palette="tab10",
